...
library(dplyr)
if(require(rnaturalearthhires) == FALSE){
...
We will use occurrence data of the fern genus *Cyathea* in Brazil, that comprises a group of fern species. This dataset is available in the `{naturalist}`. We used these dataset in order to exemplify how to use a dataset with several species in `{naturaList}`. 
...
Both spatial distribution and niche space are compared between the data before and after the cleaning process. The ratio of area after cleaning and before cleaning both spatial distribution and niche space are given for each species as one of the results of `clean_eval` function.
