...
It might occur that some strings in the 'determined.by' column of the occurrence dataset do not correspond to a taxonomist name. Strings as such `"Unknown"` often is included in the 'determined.by' data field. It is important then that such strings be ignored by the `classify_occ()`, if not this functions could flag an occurrence record as determined by a taxonomist when it was not. 

To cope with this issue, `get_det_names()` can be used to verify which strings are not taxonomists names. This function returns all different strings in the 'determined.by' column of the dataset. Based on this list of names, you could create a character vector with the strings to be ignored by `classify_occ()`, providing it to the `ignore.det.names` argument. See also the help of the function for more details(`?classify_occ`). 
...
Using the `grid_filter()` with cell size of 0.5 decimal degrees, the number of occurrences was reduced to 102. Althogh, the proportion of occurrences identified by specialists increased from 21.4 to 37.2%. 
